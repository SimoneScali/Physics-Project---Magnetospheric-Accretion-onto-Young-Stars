...
legend('Multipole Parallel', '', '', 'Star', 'Multipole Anti-Parallel', '', 'Dipole', '', Location='northeast')
...
for j = 1:3
    ...
    for i = 1:numel(B_oct_area)
        ...
        if r_null2 < RR1(j,1)
            ...
            [x1_in_para, z1_in_para, B1_in_para, theta1_para1, R1_in_para] = multipole(RR1(1,1), theta, B_dipole, B_oct_area(i), theta_dip, theta_oct(1), dS, G, R_star, M_star);
            [x1_out_para, z1_out_para, B1_out_para, theta2_para1, R1_out_para] = multipole(RR1(1,2), theta, B_dipole, B_oct_area(i), theta_dip, theta_oct(1), dS, G, R_star, M_star);
        
            [x1_in_anti, z1_in_anti, B1_in_anti, theta1_anti1, R1_in_anti] = multipole(RR1(1,1), theta, B_dipole, B_oct_area(i), theta_dip, theta_oct(2), dS, G, R_star, M_star);
            [x1_out_anti, z1_out_anti, B1_out_anti, theta2_anti1, R1_out_anti] = multipole(RR1(1,2), theta, B_dipole, B_oct_area(i), theta_dip, theta_oct(2), dS, G, R_star, M_star);
        ...
        elseif RR1(j,1) < r_null2 && r_null2 < RR1(j,2)
            ...
            [x2_in_para, z2_in_para, B2_in_para, theta1_para2, R2_in_para] = multipole(RR1(2,1), theta, B_dipole, B_oct_area(i), theta_dip, theta_oct(1), dS, G, R_star, M_star);
            ...
            [x2_in_anti, z2_in_anti, B2_in_anti, theta1_anti2, R2_in_anti] = multipole(RR1(2,1), theta, B_dipole, B_oct_area(i), theta_dip, theta_oct(2), dS, G, R_star, M_star);
...
%     title(strcat('Multipole Fall Accretion Rate Scenario  ', num2str( j)))
    ...
    if j == 2
        ...
        R_out_para = R_out_para(2568:end);
        Vff_para_out = Vff_para_out(2568:end);
        B_out_para = B_out_para(2568:end);
    ...
    ax = gca;
    hold on
    plot(R_out_para, 10+Vff_para_out./(100.*1000),'r-', LineWidth=1);
    plot(R_out_anti, 10+Vff_anti_out./(100.*1000),'b-', LineWidth=1);
    plot(R_out_dip, 10+Vff_dip_out./(100.*1000),'k-', LineWidth=1);
    grid on
    ax.FontSize = 15;
    xlabel('r/R_{*}', 'FontSize', 15)
    ylabel('V_{ff} [km/s]', 'FontSize', 15)
%     title('Free fall velocity vs Radius', 'FontSize', 15)
...
%     n_dip_in = (p_dip_in / (1e-24));
    ...
    ax = gca;
    hold on
    plot(R_out_para, log10(n_para_out),'r-', LineWidth=1);
...
%     plot(R_in_dip, log10(n_dip_in),'k--', LineWidth=1);
    ...
    grid on
    xlabel('r/R_{*}', 'FontSize', 15)
    ylabel('log(n(r) [cm^{-3}])', 'FontSize', 15)
    ax.FontSize = 15;
