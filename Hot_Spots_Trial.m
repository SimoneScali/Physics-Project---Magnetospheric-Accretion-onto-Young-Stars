...
for p = 1:numel(phi1)
    ...
    for t = 1:(numel(theta1) - 1)
        ...
        for i = 1:2 %1:numel(rm)
    

            [x_coords, y_coords, z_coords, BB1, RR1] = multipole(rm(i), theta1(t), phi1(p), gamma1(p), B_dipole, B_octupole, theta_dip, theta_oct(1), dS, R_star);
            [x_coords2, y_coords2, z_coords2, BB2, RR2] = multipole2(rm2(i), phi1(p), gamma1(p), B_dipole, B_octupole, theta_dip, theta_oct(1), dS, R_star);
            [x_coords3, y_coords3, z_coords3, BB3, RR3] = multipole3(rm2(i), phi1(p), gamma1(p), B_dipole, B_octupole, theta_dip, theta_oct(1), dS, R_star);
...
for i = 1:numel(phi2)
    ...
    [B_r, B_theta, B_phi] = field1(1, theta2, phi2, gamma2, B_dipole, B_octupole, theta_dip, theta_oct(1), R_star);
...
[x,y,z] = sphere(val);
hs1 = surf(x,y,z, flipud(BBBB)/1000, 'EdgeColor','none');
colormap("turbo");
colorbar;
q1 = get(hs1);
a = colorbar;
...
for i = 1:numel(phi2)
    ...
    BBBB2(:,i) = (B_r.^2 + B_theta.^2).^0.5;


end
% 
% [x,y,z] = sphere(val);
% hs1 = surf(x,y,z, flipud(BBBB2)/1000, 'EdgeColor','none');
% colormap("turbo");
% colorbar;
% q1 = get(hs1);
% a = colorbar;
...
tt = [-90 90];
pp = [0 360];
image(pp, -tt, flipud(BBBB/1000), 'CDataMapping', 'scaled')
...
tt = [-90 90];
pp = [0 360];
image(pp, -tt, flipud(BBBB2/1000), 'CDataMapping', 'scaled')
...
function [x_coords, y_coords, z_coords, BB, RR] = multipole(r, theta, phi, gamma, B_dipole, B_octupole, theta_dip, theta_oct, dS, R_star)
...
function [x_coords2, y_coords2, z_coords2, BB2, RR2] = multipole2(r, phi, gamma, B_dipole, B_octupole, theta_dip, theta_oct, dS, R_star)
...
function [x_coords3, y_coords3, z_coords3, BB3, RR3] = multipole3(r, phi, gamma, B_dipole, B_octupole, theta_dip, theta_oct, dS, R_star)
